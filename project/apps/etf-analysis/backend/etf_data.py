...
# Reuse HTTP connections and prevent duplicate Yahoo .info requests for the same symbol.
...
_FUNDS_OBJECTS = {}
_FUNDS_LOCKS = {}
_FUNDS_LOCKS_GUARD = threading.Lock()

def _symbol_lock(symbol: str):
    symbol = symbol.upper()
    with _INFO_LOCKS_GUARD:
        return _INFO_LOCKS.setdefault(symbol, threading.Lock())

def _get_raw_info(ticker_symbol: str) -> dict:
    # Fetch Yahoo's full info once and share it across ETF sections.
    symbol = ticker_symbol.strip().upper()
    key = f"rawinfo:{symbol}"
    cached = cache.get(key, max_age=90)
...
def _get_funds_data(ticker_symbol: str):
    # yfinance's funds_data object can serve holdings, sectors and assets. Keep
    # one warm object per symbol so those sections do not perform duplicate I/O.
    symbol = ticker_symbol.strip().upper()
    now = time.time()
    item = _FUNDS_OBJECTS.get(symbol)
    if item and now - item[0] < 21600:
        return item[1]
    with _FUNDS_LOCKS_GUARD:
        lock = _FUNDS_LOCKS.setdefault(symbol, threading.Lock())
    with lock:
        item = _FUNDS_OBJECTS.get(symbol)
        if item and time.time() - item[0] < 21600:
            return item[1]
        try:
            fd = yf.Ticker(symbol).funds_data
        except Exception:
            fd = None
        _FUNDS_OBJECTS[symbol] = (time.time(), fd)
        return fd


def get_etf_info(ticker_symbol: str) -> dict:
    cache_key = f"info:{ticker_symbol.upper()}"
    cached = cache.get(cache_key, max_age=21600)
    if cached is not None:
        return cached

    info = _get_raw_info(ticker_symbol)
...
def _fetch_holdings_from_stockanalysis(symbol: str) -> list:
    ...
    r = _HTTP.get(url, timeout=(3.5, 5.0))
...
def get_top_holdings(ticker_symbol: str) -> list:
    ...
    # Fast path: yfinance funds_data is already shared with sector/asset calls.
    # This avoids waiting on a second website during the main ETF analysis.
    holdings = []
    try:
        fd = _get_funds_data(ticker_symbol)
    ...
    # Fallback only when Yahoo does not provide holdings. The timeout is short
    # so a slow secondary provider cannot freeze the full analysis page.
    if not holdings:
        try:
...
def get_etf_metrics(ticker_symbol: str) -> dict:
    ...
    cached = cache.get(cache_key, max_age=90)
    if cached is not None:
        return cached
    ...
    result = {
        ...
        # Yahoo/yfinance reports ytdReturn as a decimal fraction (0.12 = 12%).
        "ytd_return": round(ytd_return * 100, 2) if ytd_return is not None else None,
...
def get_technical_indicators(ticker_symbol: str, period: str = "1y") -> dict:
    ...
    cached = cache.get(cache_key, max_age=900)
    if cached is not None:
        return cached
...
def get_top_movers(limit: int = 10) -> dict:
    """Get top gaining and losing ETFs using one batch Yahoo request."""
    cache_key = f"movers:{limit}"
    cached = cache.get(cache_key, max_age=90)
    if cached is not None:
        return cached

    popular_etfs = sorted(set(POPULAR_ETFS + [
        "IEMG", "SCHD", "VYM", "HDV", "NOBL", "DVY", "JEPI",
    ...
    try:
        ...
        for sym in popular_etfs:
            try:
                if hasattr(frame.columns, "nlevels") and frame.columns.nlevels > 1:
                    ...
                    # Single-ticker fallback, mainly useful for tests.
                    df = frame
                ...
                vols = df["Volume"].dropna() if "Volume" in df else []
        ...
        # The movers panel is non-critical. Return an empty result instead of
        # spending tens of seconds doing dozens of sequential quote requests.
        pass

    gainers.sort(key=lambda x: x["change_pct"], reverse=True)
    losers.sort(key=lambda x: x["change_pct"])
    result = {"gainers": gainers[:limit], "losers": losers[:limit]}
    cache.set(cache_key, result)
    return result
